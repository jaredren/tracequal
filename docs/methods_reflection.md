...
The original v1.0.0 cache was overwritten during iteration and regenerated with a second extraction. Same prompt, same model, same temperature 0, same fixture. The two runs differed in row count (22 versus 23 rows) and in four individual check outcomes (`matrix_row_count`, `decision_enum_accepted`, `decision_enum_rejected`, `low_confidence_t38` all flipped between the runs). The identical headline pass count, 12, masked four flips underneath it. The committed cache (`outputs/synthetic_long_decisions__prompt-1.0.0.json`) is the regenerated 23-row run; the earlier 22-row run was overwritten and is no longer in the repository.

The 23-row count includes one AI turn (turn 6) that produced two rows because the turn contained two distinct suggestions: a Code 3 caution and a re-recording observation. The extractor split a multi-suggestion AI turn into two rows. The schema permits this; it is defensible behavior, not a bug. It also interacts with the inclusion-bias finding above: when an AI turn is rich with content, the extractor's bias toward emitting tends to produce more rows than a stricter one-row-per-AI-turn rule would.
